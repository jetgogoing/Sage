...
async def test_simplified_flow():
    ...
    # 4. 测试简化的stop hook（模拟）
    print("\n4. 模拟stop hook处理...")
    from hooks.scripts.sage_stop_hook_simple import SimpleSageStopHook
    
    # 创建测试对话文件
    test_conversation_file = temp_dir / "test_conversation.txt"
    with open(test_conversation_file, 'w') as f:
        f.write("Human: 这是一个测试问题\n")
        f.write("Assistant: 这是助手的测试响应\n")
        f.write("使用了多个工具来完成任务。")
    
    hook = SimpleSageStopHook()
    
    # 测试提取对话
    conversation = hook.extract_conversation_from_file(str(test_conversation_file))
    if conversation:
        print(f"  提取到对话:")
        print(f"    用户: {conversation['user_input']}")
        print(f"    助手: {conversation['assistant_response'][:50]}...")
