...
#### `os.getenv('SAGE_HOME', '.')/hooks/scripts/sage_stop_hook.py`
...
#### `os.getenv('SAGE_HOME', '.')/scripts/test_message_parser_fix.py` - ✅ **测试通过**
- 测试字符串格式content解析
- 测试数组格式content解析  
- 测试tool_result格式content解析
- **结果**: 基础解析器(`_parse_claude_cli_message`) 所有测试通过

#### `os.getenv('SAGE_HOME', '.')/scripts/test_enriched_parser.py` - ❌ **测试失败**
- 用于测试增强版解析器
- **问题**: 语法错误阻止测试执行
...
File "os.getenv('SAGE_HOME', '.')/hooks/scripts/sage_stop_hook.py", line 666
