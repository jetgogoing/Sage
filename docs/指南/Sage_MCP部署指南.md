...
**注意**: `start_sage_mcp.sh` 脚本当前硬编码使用了位于 `/Users/jet/zen-mcp-server/.zen_venv/` 的虚拟环境。为提高可移植性，建议修改脚本以使用项目内的 `.venv`。我们将在步骤 4 中进行此项改进。
...
当前 `start_sage_mcp.sh` 脚本中硬编码了数据库密码和 API 密钥，这存在严重的安全风险。我们将采用行业标准的 `.env` 文件来管理这些敏感信息。

#### 4.1 创建 `.env` 文件

在项目根目录（与 `start_sage_mcp.sh` 同级）创建一个名为 `.env` 的文件。

#### 4.2 填充 `.env` 文件

将以下内容复制到 `.env` 文件中。这些值基于 `start_sage_mcp.sh` 和 `docker-compose-db.yml` 的配置。

```dotenv
# .env

# -- 数据库配置 --
# 与 docker-compose-db.yml 保持一致
DB_HOST=localhost
DB_PORT=5432
DB_NAME=sage_memory
DB_USER=sage
...
# 可选参数
SAGE_MAX_RESULTS=10
SAGE_SIMILARITY_THRESHOLD=0.75
```

#### 4.3 修改 `start_sage_mcp.sh` 脚本

编辑 `start_sage_mcp.sh` 脚本，移除硬编码的环境变量，改为从 `.env` 文件加载，并使用项目内的虚拟环境。

**修改后的 `start_sage_mcp.sh`:**
...
### 步骤 5: 启动 Sage MCP 服务器

现在，你可以通过运行修改后的脚本来启动服务器。该脚本将自动完成以下任务：

1. 检查并启动 Docker 数据库容器 (`sage-db`)
2. 等待数据库准备就绪  
3. 从 `.env` 文件加载环境变量
4. 使用项目虚拟环境中的 Python 解释器运行 `sage_mcp_stdio_single.py`
...
**路径说明**:
- 将 `/path/to/your/project/start_sage_mcp.sh` 替换为你本地项目中 `start_sage_mcp.sh` 的**绝对路径**
- `"command": "/bin/bash"` 是在 macOS/Linux 上运行 shell 脚本的标准方式。对于 Windows，你可能需要使用 `C:\\Windows\\System32\\bash.exe` (如果使用 WSL) 或其他可以执行 `.sh` 脚本的 shell
- `startupTimeout` (单位：毫秒) 建议设置为 30000 (30秒)，因为启动脚本需要等待数据库，可能会花费一些时间
